...
After a successful key purge, Blossom asks the upload service to fetch a bounded sample of the just-erased main blob URLs through the public media host (`POST /probe-delivery`) and records the aggregate result in the same timing record: `delivery_probe_checked`, `delivery_probe_present`, `delivery_probe_inconclusive`, and `delivery_probe_ms`. A `2xx` is an erasure-failure signal, because an edge cache answered with content after the object was erased; `404` is the expected absent result; any other status and any transport failure is inconclusive. The probe is best-effort and never gates the completion response: a residual edge copy cannot be repaired by retrying the vanish, and a probe observes only the POP serving its origin, so a single clean probe is weak evidence while a `2xx` is strong evidence. Only aggregate counters are recorded; failing hashes appear in operational `[VANISH] delivery_probe` diagnostics.
