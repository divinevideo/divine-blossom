...
Before erasure starts, Blossom synchronously delivers a minimal account-linked authorization record through the authenticated `/audit/vanish` endpoint. It then persists `vanish_in_progress:v1:<pubkey>` before any blob mutation. That marker has no TTL, survives interrupted requests and retries, and is removed only after account finalization and completion-audit delivery both succeed. Missing-byte reconciliation checks the marker before changing metadata and fails closed when two consecutive reads disagree or either read fails or contains malformed state. During the marker rollout transition, reconciliation applies the same checks to incomplete audit retry state so an erasure interrupted by earlier code remains protected. Because marker reads cannot atomically reserve absent state until the repair mutation, reconciliation also refuses to remove a hash that remains in any affected account blob list.

User-signed requests emit `vanish_authorized`; admin API requests emit `admin_vanish_authorized` and do not identify the target account as the actor. After account-list finalization, every open initiator record for the account receives its corresponding completion action; a missing authorization delivery is retried before that completion. Operational audit KV state is scoped by account and initiator and preserves the operation ID and each phase timestamp across retries. Initial creation is add-only, and every later transition uses a generation precondition, so concurrent requests cannot replace the operation identity or roll a completed state backward. Cloud Logging therefore receives the same insert ID and timestamp when delivery is retried and can deduplicate the entry. Active incomplete audit state expires after seven days without activity. Completed audit state remains for a one-day retry window so a lost terminal response does not create a second audit operation, then expires (Fastly may take up to 24 additional hours to delete an expired key). The records contain the account pubkey, initiator, action, audit version, operation ID, timestamp, and insert ID. User-signed records also identify the account as the actor. They do not contain the signed authorization event, content hashes, free-text reason, or aggregate timing. Only records delivered through authenticated `/audit/vanish`, not the legacy best-effort `/audit` route, carry vanish evidentiary weight. If malformed retry state blocks an account fail-closed, an operator must delete only that account and initiator's `vanish_audit:v1:<pubkey>:<initiator>` key before retrying.
