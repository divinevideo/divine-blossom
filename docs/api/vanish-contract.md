...
Before erasure starts, Blossom synchronously delivers a minimal account-linked authorization record through the authenticated `/audit/vanish` endpoint. It then persists `vanish_in_progress:v1:<pubkey>` before any blob mutation. That marker has no TTL, survives interrupted requests and retries, and is removed only after account finalization and completion-audit delivery both succeed. Missing-byte reconciliation checks the marker immediately before changing metadata and fails closed when two consecutive reads disagree or either read fails or contains malformed state.
