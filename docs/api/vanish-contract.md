...
- `fully_deleted` counts only sole-owner blobs whose main objects are confirmed absent from GCS and Fastly Object Storage (FOS), whose complete GCS hash prefixes and unshared derived audio are confirmed absent, whose erasure evidence was durably recorded, and whose required metadata cleanup and CDN purge completed. When the B2 replica gate is enabled, every B2 version and hide marker under the hash prefix must also be explicitly deleted and every configured B2-backed delivery zone purged.
...
- `pending` counts entries deferred by the per-call bound, including a B2 hash whose version set exceeds one bounded page and continues on a later call. Failed attempted entries are counted in `errors`, not `pending`, and remain discoverable for retry.
...
CDN invalidation uses concurrent batch surrogate-key purges as soon as both main origins confirm deletion. A derivative cleanup failure still blocks `fully_deleted` and remains retryable, but it does not leave a successfully deleted main object available from CDN cache. Failed storage hashes are normally retried once within the call. When B2 is enabled, the caller retries instead so both attempts do not consume the same Compute backend-request budget. `errors` counts source hashes still failed after the configured attempts, never individual provider attempts or derived-audio hashes.

The B2 replica is disabled unless `b2_replica_enabled` is true. Enabling it also makes B2 Native API version deletion, post-delete absence verification, and synchronous purge of every hostname in `bunny_delivery_zones` mandatory. Configure one or two delivery hostnames. A B2-enabled call runs one bounded wave covering at most a source blob and its derived audio, so the wave plus the call's audit and timing requests fits Fastly Compute's 32-backend-request per-execution limit. Larger version sets are erased across retry attempts before completion is reported: a hash with versions beyond the current page is counted in `pending`, not `errors`, so a drain in progress is not reported as a failure. The Fastly service must permit dynamic backends so storage calls reach the per-account API host returned by Backblaze authorization. The edge accepts only a dedicated `b2_vanish_application_key_id` / `b2_vanish_application_key` credential restricted to the configured `b2_bucket_id` with exactly `listFiles` and `deleteFiles`; the existing broad development key is not used.
...
Before erasure starts, Blossom synchronously delivers a minimal account-linked authorization record through the authenticated `/audit/vanish` endpoint. User-signed requests emit `vanish_authorized`; admin API requests emit `admin_vanish_authorized` and do not identify the target account as the actor. After account-list finalization, every open initiator record for the account receives its corresponding completion action; a missing authorization delivery is retried before that completion. Operational KV state is scoped by account and initiator and preserves the operation ID and each phase timestamp across retries. Initial creation is add-only, and every later transition uses a generation precondition, so concurrent requests cannot replace the operation identity or roll a completed state backward. Cloud Logging therefore receives the same insert ID and timestamp when delivery is retried and can deduplicate the entry. Active incomplete state expires after seven days without activity. Completed state remains for a one-day retry window so a lost terminal response does not create a second audit operation, then expires (Fastly may take up to 24 additional hours to delete an expired key). The records contain the account pubkey, initiator, action, audit version, operation ID, timestamp, and insert ID. User-signed records also identify the account as the actor. They do not contain the signed authorization event, content hashes, free-text reason, or aggregate timing. Only records delivered through authenticated `/audit/vanish`, not the legacy best-effort `/audit` route, carry vanish evidentiary weight. If malformed retry state blocks an account fail-closed, an operator must delete only that account and initiator's `vanish_audit:v1:<pubkey>:<initiator>` key before retrying.
...
After a successful key purge, Blossom asks the upload service to fetch a bounded sample of the just-erased main blob URLs through the public media host (`POST /probe-delivery`) and records the aggregate result in the same timing record: `delivery_probe_checked`, `delivery_probe_present`, `delivery_probe_inconclusive`, `delivery_probe_errors`, `delivery_probe_skipped`, and `delivery_probe_ms`. A `2xx` is an erasure-failure signal, because an edge cache answered with content after the object was erased; `404` is the expected absent result; any other status is inconclusive. `delivery_probe_errors` is `1` when the probe could not complete at all (missing credential, transport failure, deadline expiry, non-2xx response, or malformed response body), and `delivery_probe_skipped` is `1` when the call was already too close to its internal time budget to attempt the probe, so an all-zero probe result is otherwise attributable to having nothing to check. The edge bounds the complete optional RPC, including response headers and body reads, to the remaining vanish time budget. Body progress does not reset that deadline. The probe is best-effort and never gates the completion response: a residual edge copy cannot be repaired by retrying the vanish, and a probe observes only the POP serving its origin. Purge propagation to that POP is normally complete before the probe runs, but a `present` result is a signal to verify rather than proof, while a single clean probe is weak evidence. Only aggregate counters are recorded; failing hashes appear in operational `[VANISH] delivery_probe` diagnostics.
