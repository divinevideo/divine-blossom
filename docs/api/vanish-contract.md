...
Each call attempts at most 10 list entries. The edge batches only the FOS delivery replicas into multi-object deletes. It sends one authenticated Cloud Run request containing the source hashes and any unreferenced derived-audio hashes, up to 20 hashes total; Cloud Run owns all GCS deletion through Google's client. Cloud Run handles eight hashes concurrently, deletes at most 25 prefix objects per hash in one pass, and applies one 30-second handler deadline as a service resource ceiling. The production caller allows 60 seconds for the whole request and the edge's `cloud_run_upload` backend allows 120 seconds for a first byte, so the shorter inner ceiling leaves room for cold start, edge work, and response travel before the caller's budget expires and the caller receives the retryable response instead of timing out. Source and derived-audio hashes are interleaved so a slow batch does not consistently defer all derived cleanup. If more objects remain or the deadline expires, that hash returns `retryable`; later caller-driven requests continue the same idempotent cleanup. The handler deadline starts after any Cloud Run cold start and is not an edge wall-clock timeout. One slow or unexpectedly large prefix therefore cannot make a running handler unbounded. Local mode has no Cloud Run dependency and retains direct MinIO cleanup.

CDN invalidation uses concurrent batch surrogate-key purges as soon as both main origins confirm deletion. A derivative cleanup failure still blocks `fully_deleted` and remains retryable, but it does not leave a successfully deleted main object available from CDN cache. Each edge batch makes one storage attempt; failed hashes remain discoverable for the caller's next bounded request. `errors` counts source hashes failed by that attempt, never individual provider operations or derived-audio hashes.
