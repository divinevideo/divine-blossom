...
- `divine-funnelcake/database/migrations/<next>_lossless_cdn_view_capture.up.sql`: create raw delivery table, backfill from old counts, recreate `video_total_views_data_mv` to use candidate deliveries.
- `divine-funnelcake/database/migrations/<next>_lossless_cdn_view_capture.down.sql`: restore `video_total_views_data_mv` from `cdn_view_counts` while preserving the raw evidence table.
- `divine-blossom/vcl/log_cdn_views.vcl`: equivalent explicit VCL snippet for the v2 payload and broad response condition.
- `divine-blossom/docs/runbooks/cdn-view-counting.md`: operational instructions for the v2 endpoint and smoke checks.

### Task 1: Subscriber v2 Payload Parsing

**Files:**
- Modify: `divine-funnelcake/bin/cdn-view-subscriber/src/main.rs`
...
```bash
cargo test --manifest-path ../divine-funnelcake/bin/cdn-view-subscriber/Cargo.toml collect_pending_batch_routes_v2_payload_to_delivery_rows collect_pending_batch_keeps_v1_payload_on_legacy_table -- --nocapture
```

Expected: fails because `valid_message_json` and `delivery_rows` do not exist.

- [ ] **Step 3: Implement v2 row parsing**
...
```bash
cargo test --manifest-path ../divine-funnelcake/bin/cdn-view-subscriber/Cargo.toml
```
...
**Files:**
- Create: `divine-funnelcake/database/migrations/<next>_lossless_cdn_view_capture.up.sql`
- Create: `divine-funnelcake/database/migrations/<next>_lossless_cdn_view_capture.down.sql`
...
- [ ] **Step 2: Run migration-oriented checks**

Run:

```bash
rg -n "cdn_media_delivery_events|video_total_views_data_mv|cdn_view_counts" ../divine-funnelcake/database/migrations/<next>_lossless_cdn_view_capture.*
```
...
**Files:**
- Modify: `divine-blossom/vcl/log_cdn_views.vcl`
- Modify: `divine-blossom/docs/runbooks/cdn-view-counting.md`
...
```bash
git -C ../divine-funnelcake diff -- bin/cdn-view-subscriber/src/main.rs database/migrations/<next>_lossless_cdn_view_capture.up.sql database/migrations/<next>_lossless_cdn_view_capture.down.sql
git diff -- vcl/log_cdn_views.vcl docs/runbooks/cdn-view-counting.md scripts/tests/test_cdn_view_vcl.py
```
