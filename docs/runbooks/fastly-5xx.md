...
cache state, restart count, and elapsed milliseconds. It does not record a URL,
query string, client address, authorization, cookies, or body.
...
sanitization cannot change the VCL-side correlation value. `vcl/deliver.vcl`
...
- An outer VCL 5xx with no matching Compute record indicates the request failed
  before Compute completed.
