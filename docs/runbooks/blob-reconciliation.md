...
The second run reclassifies the complete file before the first mutation. It writes nothing if the live candidate count changed or exceeds the cap. Immediately before each soft delete, it re-reads the blob metadata and reverse references, then reads every affected account's durable `vanish_in_progress:v1:<pubkey>` marker twice. During the rollout transition, it also double-reads account and admin audit retry state so an erasure interrupted before the dedicated marker existed remains protected. A no-longer-active status, inconsistent metadata, malformed marker, failed read, or disagreement between paired reads prevents repair. Present in-progress state excludes the candidate until account erasure completes. The per-candidate reads are deliberately uncached so a concurrent account erasure remains visible before each mutation. Whole-catalogue repair is rejected.
...
| `missing_bytes` | Active or pending metadata, missing configured storage object, and direct public delivery returned 404. | Eligible for the guarded two-run repair protocol only when current metadata remains active and every affected account has a consistently absent in-progress erasure marker. |
