...
| `missing_bytes` | Publicly available metadata, missing configured storage object, and direct public delivery returned 404. | Eligible for the guarded two-run repair protocol only when current metadata remains active and the hash is absent from every owner or referrer blob list. |
