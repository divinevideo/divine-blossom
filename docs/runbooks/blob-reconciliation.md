...
The second run reclassifies the complete file before the first mutation. It writes nothing if the live candidate count changed or exceeds the cap. Immediately before each soft delete, it re-reads the blob metadata and reverse references, reads every affected account's durable `vanish_in_progress:v1:<pubkey>` marker twice, checks account and admin audit retry state during the rollout transition, and finally re-reads every affected account blob list. A no-longer-active status, inconsistent metadata, malformed marker, failed read, disagreement between paired reads, or current list membership prevents repair. Marker reads cannot reserve absent state until the later repair request, so list membership remains the fail-closed mutation boundary: reconciliation must not remove an entry that account erasure can still use for retry discovery. The per-candidate reads are deliberately uncached so concurrent state changes remain visible before each mutation. Whole-catalogue repair is rejected.
...
| `missing_bytes` | Active or pending metadata, missing configured storage object, and direct public delivery returned 404. | Eligible for the guarded two-run repair protocol only when current metadata remains active, every affected account has consistently absent in-progress erasure state, and the hash is absent from every affected account blob list. |
| `unverified_missing_bytes` | Active or pending metadata and missing configured storage object, but no public endpoint was probed. | Re-run with the correct public endpoint. Never repair from this class. |
