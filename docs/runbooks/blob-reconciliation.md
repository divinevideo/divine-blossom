...
The second run reclassifies the complete file before the first mutation. It writes nothing if the live candidate count changed or exceeds the cap. Immediately before each soft delete, it also re-reads the blob metadata, reverse references, and every owner or referrer blob list that the delete would mutate. A no-longer-active status, inconsistent metadata, or failed read prevents repair. A hash still in any of those lists may be retry state for an interrupted account vanish, so the tool excludes it rather than clearing the marker. List membership is also the normal state for uploaded blobs, so `excluded_vanish_retry` is expected for most otherwise repairable candidates and cannot be resolved by retrying this tool without out-of-band evidence. The per-candidate metadata, reference, and list reads are deliberately uncached: each soft delete changes list state, and a concurrent vanish must be observed before the next mutation. Whole-catalogue repair is rejected.
...
| `missing_bytes` | Active or pending metadata, missing configured storage object, and direct public delivery returned 404. | Eligible for the guarded two-run repair protocol only when current metadata remains active and the hash is absent from every owner or referrer blob list. |
...
On exit `3`, retain the private file and inspect the aggregate `repairs` counters. `excluded_vanish_retry` means the hash remained discoverable by account vanish and must not be repaired here; this is an expected, non-retryable outcome unless separate evidence establishes that removing every affected list entry is safe. `failed_vanish_retry_probe` means the safety check found changed or inconsistent metadata, or could not read current metadata, reverse-reference, or user-list state. Correct any service or authorization failure, then start again with a new read-only count-confirmation run. If current metadata remains non-active, investigate that lifecycle state separately; repeating reconciliation cannot make the candidate eligible. Repeating the repair command without a fresh first run is not recovery. Soft deletion is idempotent only through the admin API's current contract; verify the live aggregate result rather than assuming a retry completed previous work.
