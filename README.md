...
Admin and moderation endpoints accept an `X-Request-Id` header and echo it into related log lines (`[req=<id>]`) so retries and partial failures can be traced. When the outer Fastly VCL service is chained in front of Compute it pins the selected ID in `X-Divine-Edge-Request-Id`, which takes precedence; otherwise `X-Request-Id` is used. If neither is present, the leading segment of the Cloudflare `cf-ray` header is used; failing that, a short hex ID is generated. IDs are sanitized and capped at 64 characters, so common caller IDs (UUIDs, trace IDs) are preserved verbatim and longer values keep a 64-character prefix. Upstream services (e.g. the moderation service driving creator-delete) should forward a stable `X-Request-Id` across retries. `[PURGE]` logs carry the blob `sha256` as the surrogate key for cross-referencing cache purges.
