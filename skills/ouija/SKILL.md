...
2. **Progress** (`re` without `done`) — signals active work on a task,
   ...
   `ouija.send(from="your-id", to="X", message="halfway done", responds_to=47)`

3. **Complete** (`re` + `done=true`) — task finished, clears the pending reply:
   `ouija.send(from="your-id", to="X", message="result here", responds_to=47, done=true)`
...
If `ouija.send` fails with "session not found", the sender has disconnected.
You cannot deliver the reply. To clear the pending reply (which otherwise
blocks your stop hook), call:
`ouija.clear-reply(session="your-id", from="sender-id")`
</unreachable_sessions>
