...
COMPILER_OUTPUT_OWNER_PUBKEYS="${COMPILER_OUTPUT_OWNER_PUBKEYS:-}"
...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --timeout 300 \
  --max-instances 100 \
  --set-env-vars "^@@^CDN_BASE_URL=${CDN_BASE_URL}@@TRANSCODER_URL=${TRANSCODER_URL}@@TRANSCRIBER_URL=${TRANSCRIBER_URL}@@SENTRY_ENVIRONMENT=${SENTRY_ENVIRONMENT}@@COMPILER_OUTPUT_OWNER_PUBKEYS=${COMPILER_OUTPUT_OWNER_PUBKEYS}" \
